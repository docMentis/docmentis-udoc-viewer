...
export function createLoadingOverlay(showAttribution = true) {
    ...
    // Attribution logo shown below progress during loading (with tamper protection)
    ...
    if (showAttribution) {
        ...
        `;
        content.appendChild(attrStyle);

        function createAttribution(): HTMLAnchorElement {
            const el = document.createElement("a");
            el.className = attrClass;
            el.href = attrHref;
            el.target = "_blank";
            el.rel = "noopener";
            el.innerHTML = attrSvg;
            return el;
        }
        ...
        attrObserver = new MutationObserver((mutations) => {
            ...
            if (!content.contains(attribution)) {
                needsRestore = true;
            }
            if (!content.contains(attrStyle)) {
                content.appendChild(attrStyle);
            }
        ...
        attrObserver.observe(content, { childList: true, subtree: false });
