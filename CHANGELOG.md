...
- Fixed PDFs with extra data appended after the end-of-file marker failing to load. Some tools pad a file to an exact target size this way, which pushed the pointer to the document's object index further from the end of the file than it was being searched for, so those files were rejected as malformed (engine)
